...
import usePermissionStore from '@/store/router';
...
export default function (router: Router) {
  router.beforeEach((to, from, next) => {
    ...
    console.log(store.routerIsReady, router.getRoutes(), to.matched);

    if (!store.routerIsReady) {
      setTimeout(() => {
        const flatDynamicRoutes = compilerRoutes(dynamicRoutes, true);
        flatDynamicRoutes.forEach((item) => {
          router.addRoute(item);
        });
        console.log(store.routerIsReady, router.getRoutes());
        store.addRoutes();
